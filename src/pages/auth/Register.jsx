...
export default function Register() {
  ...
  const handleRegister = async (e) => {
    ...
    if (!form.fullName.trim()) return setMessage({ type: 'error', text: 'Please enter your full name.' })
    if (!form.email.trim()) return setMessage({ type: 'error', text: 'Please enter a valid email address.' })
    if (form.password.length < 8) return setMessage({ type: 'error', text: 'Password must be at least 8 characters.' })
    if (!acceptTerms) return setMessage({ type: 'error', text: 'Please agree to terms and privacy policy.' })

    setLoading(true)
    try {
      console.debug('Signing up user', { email: form.email, full_name: form.fullName, role: form.role })

      // Restore metadata: include full_name and role so DB trigger can sync profile
  ...
  options: {
    data: {
      ...
      if (error) {
        const statusText = error && error.status ? `${error.status} - ` : ''
        if (error.status && error.status >= 500) {
          setMessage({ type: 'error', text: `${statusText}${error.message}. Server-side error — check Supabase Auth logs and SMTP/email settings.` })
        } else {
          setMessage({ type: 'error', text: `${statusText}${error.message}` })
        }
        return
      }
      ...
      setMessage({ type: 'error', text: (err && err.message) || 'Unexpected server error during signup.' })
    } finally {
      setLoading(false)
    }
        ...
        <section className="flex flex-1 items-center justify-center p-8 md:p-12">
          <div className="w-full max-w-md rounded-2xl border border-white/20 bg-white p-6 shadow-lg shadow-slate-900/30 md:p-8">
            ...
            <form className="mt-6 space-y-4" onSubmit={handleRegister}>
              <Input name="fullName" value={form.fullName} onChange={handleChange} placeholder="Full Name" type="text" required />
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPassword((p) => !p)} className="absolute right-3 top-3 text-xs font-medium text-slate-500 hover:text-slate-700">
              ...
              <div>
                ...
                <div className="h-2 w-full rounded-full bg-slate-200">
                  <div className={`h-2 rounded-full ${strengthColors[passwordStrengthScore()]} transition-all duration-300`} style={{ width: `${(passwordStrengthScore() / 4) * 100}%` }} />
                </div>
                <div className="mt-1 text-xs font-semibold text-slate-600">{form.password ? strengthLabels[passwordStrengthScore()] : 'Type a password'}</div>
              </div>

              <div className="space-y-2">
                <label className="block text-sm font-medium text-slate-700" htmlFor="role">Select role</label>
                <select id="role" name="role" value={form.role} onChange={handleChange} className="w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 outline-none transition duration-150 focus:border-sky-500 focus:ring-2 focus:ring-sky-100">
              ...
              <label className="flex items-start gap-2 text-sm text-slate-700">
                <input type="checkbox" checked={acceptTerms} onChange={(e) => setAcceptTerms(e.target.checked)} className="mt-1 h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-400" />
                <span>
                  I agree to the{' '}
                  <a href="#" className="font-semibold text-sky-600 hover:text-sky-700" onClick={(e) => e.preventDefault()}>Terms</a>{' '}
                  and{' '}
                  <a href="#" className="font-semibold text-sky-600 hover:text-sky-700" onClick={(e) => e.preventDefault()}>Privacy Policy</a>.
                </span>
              </label>
            ...
            <p className="mt-6 text-center text-sm text-slate-500">
              Already registered?{' '}
              <Link to="/auth/login" className="font-semibold text-sky-600 hover:text-sky-700">Sign in</Link>
            </p>
          </div>
        </section>
      </div>

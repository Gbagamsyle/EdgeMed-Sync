...
export default function EditPatient() {
  ...
  useEffect(() => {
    void getPatientById(id).then(({ data, error }) => {
      if (error) {
        console.error(error)
        setStatus({ type: 'error', message: 'Failed to load patient' })
        return
      }
      const p = data
      setForm({
  ...
  const handleSubmit = async (e) => {
    ...
    setStatus({ type: 'success', message: 'Patient updated.' })
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-4xl rounded-3xl border border-slate-200 bg-white p-6 space-y-6 shadow-xl shadow-slate-200/50">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-slate-500">Make necessary changes and save to update the patient record.</p>

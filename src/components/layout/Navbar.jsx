...
export default function Navbar() {
  ...
  const displayName = profile?.full_name || user?.user_metadata?.full_name || user?.email || 'Staff member'
    ...
    <header className="fixed top-0 left-0 right-0 z-50 flex h-24 items-center justify-between border-b border-sky-700 bg-sky-600 px-6 shadow-lg">
      ...
      <div className="flex items-center gap-4">
        <div className="rounded-md px-3 py-2 text-right text-sm">
          ...
          <p className="text-sky-200">{displayRole ? displayRole.replace('_', ' ').replace(/\b\w/g, (letter) => letter.toUpperCase()) : 'Staff member'}</p>
        </div>
        <button
